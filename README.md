...
Релиз: [Releases](https://github.com/scarrymany/tg-manager/releases/tag/v1.0.0-windows)

1. Скачайте `TG-Manager-1.0.0-windows.zip`.
2. Распакуйте в любую папку (программа **не пишет в AppData** — всё рядом с exe).
3. Запустите `TGManager.exe`.

```
TG-Manager\
  TGManager.exe          ← запуск
  README.txt
...
> Windows Defender иногда ругается на упакованный exe (PyInstaller). Это
> ложное срабатывание. Добавьте папку программы в исключения.

## Возможности

- 🪟 **1 карточка = 1 контейнер.** Имя, цвет, папка, `tdata`.
- ▶️ **Запуск / стоп**, живой статус. Много окон сразу (`-many`, `-workdir`).
...
- 🧹 **Чистка аккаунта** из tdata (Telethon + opentele): каналы, группы,
  ...
  чтобы не словить `AUTH_KEY_DUPLICATED`.
...
- **HTTP** — локальный SOCKS5-мост (внутри программы) → HTTP CONNECT.
...
Нужен Python 3.10+ (галочка «Add to PATH»).
...
Готово: `dist\TG-Manager\TGManager.exe`.

CI на этой ветке (GitHub Actions, `windows-latest`) собирает zip и публикует
релиз `v1.0.0-windows` при каждом пуше в `windows`.
...
- **Прокси не применился** — скачайте прокси-обёртку; для HTTP нужен живой менеджер
  (мост крутится рядом).
- **Аккаунт не запускается** — в папке должна быть именно `tdata`, не `tdata\tdata`.
- **Defender** — исключение на папку программы.
